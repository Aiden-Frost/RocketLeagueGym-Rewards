...
class PPOLearner(object):
    def __init__(
        ...
        print(f"[rank {rank}] about to wrap policy in DDP on cuda:{local_gpu}")
        self.policy = DDP(self.policy, device_ids=[self.local_gpu], output_device=self.local_gpu)
        self.value_net = DDP(self.value_net, device_ids=[self.local_gpu], output_device=self.local_gpu)
        print(f"[rank {rank}] DDP wrapped on cuda:{local_gpu}")
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in batches:
                ...
                for minibatch_slice in range(0, self.batch_size, self.mini_batch_size):
                    ...
                    log_probs, entropy = self.policy.module.get_backprop_data(obs, acts)
