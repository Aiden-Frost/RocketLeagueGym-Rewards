...
class Learner(object):
    def __init__(
        ...
        print(f"[rank {self.rank}] envs up, about to init PPO")
        if self.rank == 0:
    ...
    def _learn(self):
        ...
        while self.agent.cumulative_timesteps < self.timestep_limit:
            ...
            t0 = time.time()
            if self.rank == 0:
                print("===============>>>", f"Starting collecting time steps")
            ...
            if self.rank == 0:
                print("===============>>>", f"[{time.time()-t0:5.2f}s] collected {steps_collected} steps")
            ...
            if self.rank == 0:
                print("===============>>>", f"epoch {self.epoch} done, buffer={self.experience_buffer.states.shape}")
